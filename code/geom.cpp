...
struct P { //点 or 向量
  ...
  P operator+(P p) { return P(x + p.x, y + p.y); } //向量相加
  P operator-(P p) { return P(x - p.x, y - p.y); } //向量相减
  ...
  double dot(P p) { return x * p.x + y * p.y; } //点积
  double det(P p) { return x * p.y - y * p.x; } //叉积
...
vector<P> isCL(P o, double r, P p1, P p2) {
  ...
  double x = (p1 - o).dot(p2 - p1), y = (p2 - p1).abs2(),
...
P circumCenter(P a, P b, P c) {
  P bb = b - a, cc = c - a;
  double double = bb.abs2(), dc = cc.abs2(), d = 2 * bb.det(cc);
  return a - P(bb.y * dc - cc.y * double, cc.x *double - bb.x * dc) / d;
}
