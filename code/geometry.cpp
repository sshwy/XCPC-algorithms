...
namespace cg {
  ...
  struct Ln {
    ...
    Vec dir() const { return (ed - st).dir(); }
  };
  typedef Ln Seg;
  ...
  Pt intersection(Ln a, Ln b) {
    assert(!parallel(a, b));
    Vt x = det(a.st - b.st, b.ed - b.st);
    Vt y = det(b.ed - b.st, a.ed - b.st);
    return a.st + (a.ed - a.st) * x / (x + y);
  }
  // 判断两个线段是否相交（含边界）
  bool check_segment_intersection(Seg a, Seg b) {
    if (cg::coincident(a, b)) {
      ...
      Pt o = cg::intersection(a, b);
      if (cg::on_seg(a, o) && cg::on_seg(b, o)) return true;
      else return false;
    }
  }
  // 两个点的距离
   ...
   */
  int polygon_point_containment(vector<Pt> g, Pt p) {
   ...
   */
  int check_circle_intersection(Cir a, Cir b) {
    Vt d = dst(a.o, b.o);
   ...
   */
  int check_circle_point_containment(Cir c, Pt p) {
    Vt d = dst(c.o, p);
    if (lt(d, c.r)) return 0;
    if (eq(d, c.r)) return 1;
    return 2;
  }
  // 求三角形内切圆 @return 一个 Cir 表示内切圆
  Cir incircle(Pt a, Pt b, Pt c) {
    ...
    Pt o = intersection(C + shiftC, B + shiftB);
    return Cir(o, r);
  }
  // 求三角形外接圆 @return 一个 Cir 表示外接圆
  Cir outcircle(Pt a, Pt b, Pt c) {
    Vec vc = r90_clockwise(a - b), vb = r90_clockwise(a - c);
    Pt mc = (a + b) / Vt(2), mb = (a + c) / Vt(2);
    Pt o = intersection(Ln(mc, mc + vc), Ln(mb, mb + vb));
    Vt r = (o - a).length();
    return Cir(o, r);
  }
  // 圆点到直线的距离
   ...
   */
  pair<Pt, Pt> circle_line_intersection(Ln L, Cir c) {
    Vt d = dst(L, c);
    d = sqrt(max(Vt(0), c.r * c.r - d * d));
    Vec shift = L.dir() * d;
    Pt mid = proj(L, c.o);
   ...
   * @return 一个 pair 表示两个交点
   */
  pair<Pt, Pt> circle_intersection(Cir c1, Cir c2) {
   ...
   */
  pair<Pt, Pt> circle_point_tangent(Cir c, Pt p) {
   ...
   */
  Pt circle_outer_homothetic_center(Cir c1, Cir c2) {
    assert(!eq(c1.r, c2.r));
   ...
   */
  Pt circle_inner_homothetic_center(Cir c1, Cir c2) {
   ...
   */
  pair<Ln, Ln> circle_outer_common_tangent(Cir c1, Cir c2) {
    assert(check_circle_intersection(c1, c2) != 0);
    if (!eq(c1.r, c2.r)) {
      Pt p = circle_outer_homothetic_center(c1, c2);
      ...
      if (pt.first == pt.second) {
        ...
        return make_pair(Ln(p, pt.first), Ln(p, pt.second));
      }
    } else {
   ...
   */
  pair<Ln, Ln> circle_inner_common_tangent(Cir c1, Cir c2) {
    assert(check_circle_intersection(c1, c2) >= 3);
    Pt p = circle_inner_homothetic_center(c1, c2);
    auto pt = circle_point_tangent(c1, p);
    if (pt.first == pt.second) {
      Vec oo = r90_clockwise(c1.o - c2.o);
      Ln t(p + oo, p);
  ...
  // 求两圆所有公切线，去重
  vector<Ln> circle_common_tangent(Cir c1, Cir c2) {
    vector<Ln> res;
    int typ = check_circle_intersection(c1, c2);
    if (typ > 0) {
      auto pt = circle_outer_common_tangent(c1, c2);
      res.pb(pt.first);
      if (pt.first.ed != pt.second.ed) res.pb(pt.second);
    }
    if (typ >= 3) {
  ...
  Vt circular_segment_area(Vt r, Vt angle) {
    return r * r * (angle - sin(angle)) / Vt(2);
  }
  // 求两个圆交面积
  Vt circle_intersection_area(Cir c1, Cir c2) {
    Vt ans = 0;
    ...
    if (typ <= 1) {
      ...
      Pt p = intersection(Ln(c1.o, c2.o), Ln(pt.first, pt.second));
