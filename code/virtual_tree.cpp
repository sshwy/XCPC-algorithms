...
int n, dep[N], fa[N][20], dfn[N], totdfn;
vector<int> g[N];
...
int dst(int x, int y) { return dep[x] + dep[y] - dep[lca(x, y)] * 2; }

vector<pair<int, int>> vt[N];
void VT_ae(int u, int v, int w) { vt[u].pb({v, w}), vt[v].pb({u, w}); }
...
void build_VT(vector<int> &V) { //建虚树并把点集更新到V里
  ...
  for (int u : V) {
    ...
    while (tp > 1 && dep[s[tp - 1]] >= dep[z])
      VT_ae(s[tp], s[tp - 1], dst(s[tp], s[tp - 1])), --tp;
    if (s[tp] != z) VT_ae(s[tp], z, dst(s[tp], z)), s[tp] = z, Vt.pb(z);
    s[++tp] = u;
  }
  while (tp > 1) VT_ae(s[tp], s[tp - 1], dst(s[tp], s[tp - 1])), --tp;
  V.pb(s[1]);
